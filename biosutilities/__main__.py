...
from biosutilities.common.paths import (
    delete_dirs, extract_folder, is_dir_read, is_empty_dir, is_file_read,
...
class BIOSUtilities:
    ...
    def __init__(self) -> None:
        ...
        main_argparser.add_argument('-u', '--utility', help='utility class name')

        self.main_arguments: Namespace = main_argparser.parse_args()

        self._utilities_classes: list[type[BIOSUtility]] = [
    ...
    def _setup_utilities_classes(self) -> None:
        if self.main_arguments.utility:
            utilities_mapping: dict[str, type[BIOSUtility]] = {
                util_cls.__name__: util_cls for util_cls in self._utilities_classes
            }

            utility_class: type[BIOSUtility] | None = utilities_mapping.get(self.main_arguments.utility)

            if not utility_class:
                raise ValueError(f'Unknown utility class name "{self.main_arguments.utility}", '
                                 f'expected one of: {", ".join(utilities_mapping)}')

            self._utilities_classes = [utility_class]
    ...
    def run_main(self, padding: int = 0) -> None:
        ...
        for input_file in self._input_files:
            ...
            for utility_class in self._utilities_classes:
                ...
                utility: BIOSUtility = utility_class(
                    input_object=input_file, extract_path=extract_path, padding=padding + 8
                )
